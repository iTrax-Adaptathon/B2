...
exports.getMonthlyOffset = async (req, res) => {
  try {
    ...
    // Calculate trees required for MONTHLY offset 
...
exports.generateMonthlyReport = async (req, res) => {
  try {
    ...
    doc.fontSize(12).font("Helvetica")
      .text(`Total CO2 Emitted: ${(Math.round(totalCO2 * 100) / 100).toFixed(2)} kg`)
    ...
    if (Object.keys(activityBreakdown).length > 0) {
      Object.entries(activityBreakdown).forEach(([type, data]) => {
        ...
        doc.fontSize(11).font("Helvetica")
          .text(`• ${type.charAt(0).toUpperCase() + type.slice(1)}: ${data.totalCO2.toFixed(2)} kg CO2 (${data.count} activities, ${percentage}%)`);
      });
    } else {
    ...
    if (recentActivities.length > 0) {
      recentActivities.forEach((activity, index) => {
        ...
        doc.fontSize(10).font("Helvetica")
          .text(`${index + 1}. ${type.charAt(0).toUpperCase() + type.slice(1)} - ${footprint} kg CO2 (${date})`);
      });
    ...
    doc.fontSize(11).font("Helvetica")
      .text(`• Plant ${monthlyTreesRequired} trees (1 tree absorbs ~21kg CO2/year)`)
      .text(`• Invest ₹${renewableEnergyCost} in renewable energy projects`);
    doc.moveDown(0.5);

    doc.fontSize(12).font("Helvetica-Bold").text("Yearly Projection:");
    doc.fontSize(11).font("Helvetica")
      .text(`• Projected yearly emissions: ${(Math.round(yearlyCO2Projection * 100) / 100).toFixed(2)} kg CO2`)
